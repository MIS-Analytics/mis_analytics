...
Docs: https://MIS-Analytics.github.io/mis_analytics/etl.html.md"""

# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/10_etl.ipynb.

# %% auto #0
__all__ = ['get_demo_data', 'StepMeta', 'format_timedelta', 'print_step_name', 'print_time', 'print_sample_from_meta_dict',
           'display_sample_from_df', 'space_strings', 'print_shape_change', 'print_step_description', 'print_step_info',
           'pipeline', 'track']
...
def print_sample_from_meta_dict(step_meta: StepMeta, mode='in'):
    for part in ('head', 'sample', 'tail'):
        print(f'{part}:')
        print(getattr(step_meta, f'{mode}_df_{part}'))

# %% ../nbs/10_etl.ipynb #c932d349
def display_sample_from_df(df):
    for part, sample in (('head', df.head(3)), ('sample', df.sample(min(5, df.shape[0]))), ('tail', df.tail(3))):
        print(f'{part}:')
        print(sample)
...
def track(func):
    @wraps(func)
    def wrapper(in_df, *args, vrbs=False, **kwargs):
        if vrbs:
            # Capture "before" data
